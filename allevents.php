...
?>
<?php
if(isset($_POST['search'])||$s!='0')
      {
        if(isset($_POST['search']))
          $s=$_POST['search'];
        $query = "SELECT id,eventname,YEAR(eventdate) AS year, MONTH(eventdate) AS month, DAY(eventdate) AS day,message FROM event where userid = $id and eventname like '%$s%' order by eventdate asc";
      }
        else
{
...
<head>
  <meta http-equiv="refresh" content="<?php echo $sec ?>; URL='<?php echo $page; ?>?check=<?php echo $s ?>'">
